...
async function update(postId) {
  const response = await fetch(`http://localhost/post/${postId}`);
  ...
  if (updatedTitle && updatedContent && updatedCategory) {
    try {
      const token = window.localStorage.getItem('authorization');
      const updateresponse = await fetch(`http://localhost/post/${postId}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
...
async function remove(postId) {
  ...
  try {
    const response = await fetch(`http://localhost/${postId}`);

    const confirmDelete = confirm('정말로 이 게시글을 삭제하시겠습니까?');
    if (confirmDelete) {
      const deleteResponse = await fetch(`http://localhost/post/${postId}?userId=${logInUserId}`, {
        method: 'DELETE',
        headers: {
...
async function like(postId, posts) {
  try {
    ...
    const token = window.localStorage.getItem('authorization');
    const response = await fetch(`http://localhost/post/${postId}/like`, {
