...
const socket = io('https://chunsik.store:3000/friendDM', { auth: { token: token } });
...
function handleImageSubmit(event) {
  ...
  socket.on('receiveUserInfo', userInfo => {
    ...
    fetch(`https://chunsik.store:3000/dm/file?dmRoomId=${dmRoomId}&userId=${userId}`, {
      method: 'POST',
      body: data,
      credentials: 'include',
    })
...
function history(dmRoomId) {
  fetch(`https://chunsik.store:3000/dm/history/${dmRoomId}`, {
    headers: {
